...
Game.prototype = {
   ...
   *
   */
  generateEncounters: function(adventure){
    ...
    while ( used.length < num_of_encounters ){
      ...
      if ( used.indexOf( rand ) === -1 ) {
        ...
        var stub = _.clone( filtered_encounters[rand] );
        ...
        // create fail effects
        encounter.addFailEffect( {value: 1, type: 'dmg'} );
        encounter.addFailEffect( {value: 10, type: 'xp'} );

        // create success effects
        encounter.addSuccessEffect( {value: 1, type: 'heal'} );
        encounter.addSuccessEffect( {value: 17, type: 'xp'} );
   ...
   * @param context
   */
  getActions: function( obj, context ) {
    if ( obj.actions ) {
  ...
  doEffect: function( effect ){
    if ( effect.type ) {
      switch ( effect.type ){
        ...
        case 'dmg':
          this.character.hp -= effect.value;
          this.output('You were damaged for ' + effect.value + ' HP!');
          break;
        ...
        case 'output':
          this.output( effect.value );
          break;

        case 'xp':
          this.character.xp += parseInt( effect.value );
          this.output('You gained ' + effect.value + ' XP!');
          break;

        case 'gp':
          this.character.gp += parseInt( effect.value );
          this.output('You gained ' + effect.value + ' GOLD!');
          break;

        case 'buff':
          this.character.addBuff( effect.value );
          this.output(effect.value.value + ' to your ' + effect.value.attribute + '!');
          break;
   ...
   */
  startAdventure: function( adventureFileName ){
 ...
 * @returns {Game}
 */
Game.create = function( req ){
  var game = new Game();

  game.input = req.systemParsedInput;
